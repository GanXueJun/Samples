...
MKDIR "%RELEASE_PATH%\data\models\yolov3"
COPY /Y "%BUILD_SCRIPT_PATH%\data\models\yolov3\coco.names" "%RELEASE_PATH%\data\models\yolov3"

IF NOT EXIST "%BUILD_SCRIPT_PATH%\data\models\yolov3\*.om" (
    ECHO [%date:~0,10%-%time:~0,8%][WARNING] No .om file in %BUILD_SCRIPT_PATH%\data\models\yolov3, please convert the model to .om file first. >> build.log
) ELSE (
    COPY /Y "%BUILD_SCRIPT_PATH%\data\models\yolov3\*.om" "%RELEASE_PATH%\data\models\yolov3"
)
