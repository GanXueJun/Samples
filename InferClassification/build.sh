...
function build_a300() {
    ...
    # check aclopExecuteV2 whether exist
    local acl_op_header="${ASCEND_HOME}/${ASCEND_VERSION}/${ARCH_PATTERN}/include/acl/acl_op.h"
    local interface_num=$(cat "${acl_op_header}" |grep aclopExecuteV2 |wc -l)
    if [ ${interface_num} -ne 0 ]; then
        export USE_ACL_OP_EXECUTE_V2=1
    fi
...
om_num=$(ls ./data/models/resnet/*.om 2>/dev/null |wc -l)
if [ ${om_num} -eq 0 ]; then
...
om_num=$(ls ./data/models/single_op/*.om 2>/dev/null |wc -l)
if [ ${om_num} -eq 0 ]; then
